...
class PatchMergingSpatialAttention(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        k_sizes = [1, 5, 9]
        dilations = [1, 1, 1]
        print("PMSA k_size:", k_sizes)
        print("PMSA dilations", dilations)

        self.layer1 = BasicConv(2 * dim, 1, kernel_size=k_sizes[0], stride=1, dilation=dilations[0], padding=(dilations[0]*(k_sizes[0]-1)+1)//2, relu=False)
        self.layer2 = BasicConv(2 * dim, 1, kernel_size=k_sizes[1], stride=1, dilation=dilations[1], padding=(dilations[1]*(k_sizes[1]-1)+1)//2, relu=False)
        self.layer3 = BasicConv(2 * dim, 1, kernel_size=k_sizes[2], stride=1, dilation=dilations[2], padding=(dilations[2]*(k_sizes[2]-1)+1)//2, relu=False)
        # self.conv = nn.Conv2d(2 * dim, 1, kernel_size=11, stride=1, padding=5)
        # self.conv1x1 = nn.Conv2d(3, 1, kernel_size=7, stride=1, padding=3)
    ...
    def forward(self, x, H, W):
        ...
        x = x.view(B, H//2, W//2, 2*C).permute(0, 3, 1, 2)
...
class PatchEmbedSpatialAttention(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=nn.LayerNorm):
        ...
        k_sizes = [1, 5, 9]
        dilations = [1, 1, 1]
        print("PESA k_size:", k_sizes)
        print("PESA dilations", dilations)

        self.layer1 = BasicConv(embed_dim, 1, kernel_size=k_sizes[0], stride=1, dilation=dilations[0], padding=(dilations[0]*(k_sizes[0]-1)+1)//2, relu=False)
        self.layer2 = BasicConv(embed_dim, 1, kernel_size=k_sizes[1], stride=1, dilation=dilations[1], padding=(dilations[1]*(k_sizes[1]-1)+1)//2, relu=False)
        self.layer3 = BasicConv(embed_dim, 1, kernel_size=k_sizes[2], stride=1, dilation=dilations[2], padding=(dilations[2]*(k_sizes[2]-1)+1)//2, relu=False)
        # self.conv = nn.Conv2d(embed_dim, 1, kernel_size=11, stride=1, padding=5)
        # self.conv1x1 = nn.Conv2d(3, 1, kernel_size=7, stride=1, padding=3)
        self.sigmoid = nn.Sigmoid()

        # Style-based recalibration module
        # self.cfc = nn.Parameter(torch.Tensor(embed_dim, 2))
    ...
    def forward(self, x):
        ...
        # x2 = self.layer2(x)
        # x3 = self.layer3(x)
        # y = self.fuse_weight_1 * x1 + self.fuse_weight_2 * x2 + self.fuse_weight_3 * x3
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,inplanes: int,planes: int,stride: int = 1,downsample = None,groups: int = 1,
                 base_width: int = 64,dilation: int = 1, norm_layer = None) -> None:
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        # patch embedding with spatial attention module
        self.patch_embed = PatchEmbedSpatialAttention(patch_size=4, in_chans=in_chans, embed_dim=embed_dim)
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                                ...
                                norm_layer=norm_layer,
                                downsample=PatchMergingSpatialAttention if (i_layer < self.num_layers - 1) else None,
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i != self.num_layers - 1:
                conv_block = getattr(self, f'layer{i+2}')
                out = conv_block(out)
                # x = x.view(-1, Wh, Ww, self.num_features[i] * 2).permute(0, 3, 1, 2).contiguous() # delete this
                x = out + x
                x = x.flatten(2).transpose(1, 2)
        return outs
...
class RMFormer(nn.Module):
    def __init__(self, config):
        super(RMFormer, self).__init__()
